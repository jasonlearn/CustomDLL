...
BOOL APIENTRY DllMain(HINSTANCE hInstance		/* library instance handle */,
	DWORD fdwReason			/* Reason this function is being called */,
	LPVOID pvReserved		/*Not used. */)
{
	hwnd = hInstance;
	switch (fdwReason)
	{
...
//Jason Chan - Used to launch dialog in C#
EXPORT HWND CALLBACK ShowRecordBox() {
	HWND recordBox = CreateDialog(hwnd, TEXT("Record"), NULL, DlgProc);
	return recordBox;
}
//method to export PBYTE to c#
EXPORT PBYTE* CALLBACK ExportSound() {
	return newBuffer;
}

//method to export legnth of the sound to C#
...
EXPORT BOOL CALLBACK DlgProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	dataLength = &dwDataLength;
	newBuffer = &pSaveBuffer;
